...
class FoldedLinesManager {
  ...
  public isFolded(range: Range, editor: TextEditor): boolean | undefined {
    const { visibleRanges } = editor;

    if (!visibleRanges.some(visibleRange => visibleRange.contains(range.start))) {
      return undefined;
    }
 ...
 * 
 * Changes:
 * added return value undefined for isFolded()
 */
