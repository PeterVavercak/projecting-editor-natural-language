...
export default class JsxRangesProvider extends BetterFoldingRangeProvider {
  ...
  private visit(node: unknown, jsxElements: TSESTree.JSXElement[]) {
  ...
  private isBaseNode(node: unknown): node is TSESTree.Node {
    return Boolean(node) && node!.hasOwnProperty("type");
  }

  private isJsxElement(node: TSESTree.Node): node is TSESTree.JSXElement {
  ...
  private getStartColumn(jsxElement: TSESTree.JSXElement): number | undefined {
  ...
  private getCollapsedText(jsxElement: TSESTree.JSXElement, document: TextDocument): string {
  ...
  private getFoldedLinesCountCollapsedText(jsxElement: TSESTree.JSXElement): string {
